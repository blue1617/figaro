...
/**
  ...
  * ten people in the database, Alice has an age represented by an average of 18 and a variance of 1. All the other
  * are uniformly distributed people named John with an age from 15 to 51. The observed average age is between 20.25
  * and 23
  */
class FifthAttacker extends Attacker {

  override val names: Seq[Name] = List("John")
  ...
  override def generateAttacker(i: Int): Element[(Name, Age)] = {
    for {
      ...
      yield (n, a)
    // yield  n -> a //NOTE from Raul: Syntactic sugar for the line above
  }
}
